...
import { ArrowLeft, ArrowRight, MapPin, Package, Layers, PhoneCall, Mail, ArrowUpRight } from 'lucide-react'
...
export default function MaterialDetail() {
  ...
  useEffect(() => {
    if (!slug) return
    setLoading(true); setNotFound(false); setMaterial(null)
    ...
    return (
      <main className="min-h-screen bg-[#020617] pt-24 pb-20 flex items-center justify-center">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-bold text-white mb-3">{t('materialDetail.notFound')}</h1>
          <p className="text-gray-400 mb-8">{t('materialDetail.notFoundDesc')}</p>
          <Link to="/materials" className="inline-flex items-center gap-2 text-amber-400 hover:text-amber-300 font-medium transition-colors">
            ...
            {t('materialDetail.backToMaterials')}
          </Link>
        </div>
      </main>
    )
  ...
  const displayName    = isAr && material.name_ar ? material.name_ar : material.name
  const displayDesc    = isAr && material.description_ar ? material.description_ar : material.description
  const displayFullDesc = isAr && material.full_description_ar ? material.full_description_ar : material.full_description
  const categoryLabel  = t(`materials.categories.${material.category}`, { defaultValue: material.category })
    ...
    <>
      ...
      <main className="min-h-screen bg-[#020617] pb-20">
        ...
        <div className="relative h-72 md:h-96 overflow-hidden">
          {material.image_url && (
            <img src={material.image_url} alt={displayName}
          ...
          <div className="absolute inset-0 flex flex-col justify-end max-w-6xl mx-auto px-6 pb-10">
            <Link to="/materials" className="inline-flex items-center gap-2 text-gray-400 hover:text-amber-400 text-sm mb-4 transition-colors w-fit">
              <BackIcon size={14} /> {t('materialDetail.backToMaterials')}
            </Link>
            ...
            <h1 className="text-4xl md:text-5xl font-black text-white mb-3">{displayName}</h1>
            {displayDesc && (
              <p className="text-gray-300 text-base md:text-lg max-w-2xl leading-relaxed">{displayDesc}</p>
            )}
        ...
        <div className="max-w-6xl mx-auto px-6 mt-12">
          <div className="grid md:grid-cols-3 gap-10">
            ...
            <div className="md:col-span-2 space-y-10">
              {displayFullDesc && (
                <div>
                  <h2 className="text-xl font-bold text-white mb-4">{t('materialDetail.aboutMaterial')}</h2>
                  <div className="space-y-4">
                    {displayFullDesc.split('\n').filter(Boolean).map((para, i) => (
              ...
              {applications.length > 0 && (
                <div>
                  <h2 className="text-xl font-bold text-white mb-4">{t('materialDetail.keyApplications')}</h2>
              ...
              {material.origin_countries && (
                <div>
                  <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                    <MapPin size={18} className="text-amber-400" /> {t('materialDetail.originSourcing')}
                  </h2>
                ...
                <div className="space-y-3 border-t border-white/5 pt-4">
                  {material.min_order && (
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-xs text-gray-500 uppercase tracking-wide mb-0.5">{t('materialDetail.minOrder')}</p>
                  ...
                  {material.packaging && (
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-xs text-gray-500 uppercase tracking-wide mb-0.5">{t('materialDetail.packaging')}</p>
          ...
          {specs.length > 0 && (
            <div className="mt-14">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-white">{t('materialDetail.technicalSpecs')}</h2>
              </div>
              <div className="bg-[#0d1424] border border-white/5 rounded-2xl overflow-hidden">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/5">
                      <th className="px-6 py-4 text-start text-gray-400 font-medium uppercase tracking-wider text-xs w-1/3">{t('materialDetail.property')}</th>
                      <th className="px-6 py-4 text-start text-gray-400 font-medium uppercase tracking-wider text-xs">{t('materialDetail.value')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {specs.map((spec, i) => (
          ...
          {related.length > 0 && (
            <div className="mt-16 border-t border-white/5 pt-14">
              <h2 className="text-2xl font-bold text-white mb-8">{t('materialDetail.otherMaterials')}</h2>
              <div className="grid md:grid-cols-3 gap-4">
                {related.map(m => {
                  const rName = isAr && m.name_ar ? m.name_ar : m.name
                  const rDesc = isAr && m.description_ar ? m.description_ar : m.description
                  return (
                    <Link key={m.slug} to={`/materials/${m.slug}`}
                      ...
                      {m.image_url && (
                        <div className="w-full h-28 mb-4 rounded-xl overflow-hidden border border-white/5">
                          <img src={m.image_url} alt={rName} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                        </div>
                      )}
                      ...
                      <h3 className="text-white font-bold mb-1">{rName}</h3>
                      <p className="text-gray-500 text-xs leading-relaxed line-clamp-2">{rDesc}</p>
                    </Link>
                  )
                })}
              </div>
